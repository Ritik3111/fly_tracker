...
lint:  ## run static analysis with flake8
	python -m black --check fly_tracker setup.py
	python -m flake8 fly_tracker setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black fly_tracker/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./fly_tracker

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v fly_tracker/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v fly_tracker/tests --cov=fly_tracker --cov-branch --cov-fail-under=75 --cov-report term-missing 
...
TMPREPO=/tmp/docs/fly-tracker

pages: 
	rm -rf $(TMPREPO)
	git clone -b gh-pages git@github.com:Ritik3111/fly-tracker.git $(TMPREPO)
	rm -rf $(TMPREPO)/*
